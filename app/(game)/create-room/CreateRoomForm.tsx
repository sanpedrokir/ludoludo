...
export default function CreateRoomForm({ defaultStake = false }: { defaultStake?: boolean }) {
  ...
  return (
    <div className="px-6 py-8 flex flex-col gap-6 max-w-md mx-auto w-full">
      ...
      <form action={formAction} className="flex flex-col gap-5">
        ...
        {/* Stake game — only shown when entering via High Stakes button */}
        {defaultStake && (
          <div className="rounded-2xl border-2 border-amber-500 bg-amber-100 p-4">
            <input type="hidden" name="isStakeGame" value="true" />
            <div className="text-sm font-black text-amber-900 mb-1">🎰 High Stakes Game</div>
            <div className="text-xs text-amber-700">
              Each player pays <span className="font-black">$250,000</span> to enter. Winner takes the entire pot.
            </div>
          </div>
        )}
